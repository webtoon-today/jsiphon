...
describe('Junk Handling', () => {
    ...
    describe('junk after JSON', () => {
        it('ignores text after closing brace', async () => {
            const results = await parseChunks<{ name: string }>(['{"name": "test"} and some trailing text']);
            expect(results[0].name).toBe('test');
            expect(isAmbiguous(results[0][META].ambiguous)).toBe(false);
        });

        it('ignores text after closing bracket', async () => {
            const results = await parseChunks<number[]>(['[1, 2, 3] extra stuff here']);
            expect(results[0]).toEqual([1, 2, 3]);
        });
    ...
    describe('junk in streaming', () => {
        it('handles junk arriving in separate chunk', async () => {
            const results = await parseChunks<{ x: number }>(['blah blah ', '{"x": 1}']);
            expect(results[1].x).toBe(1);
        });

        it('handles closing and junk in separate chunks', async () => {
            const results = await parseChunks<{ a: number }>(['{"a": 1}', ' trailing']);
    ...
    describe('edge cases', () => {
        it('handles braces/brackets in junk text', async () => {
            ...
            const results = await parseChunks<{ valid: boolean }>(['text with } and ] chars {"valid": true}']);
